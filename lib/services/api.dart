...
Future<Object> signUp(
    String email, String password, List<double> faceDataList) async {
  int maxRetries = 5;
...
Future<void> logIn(String email, String password, List<double> faceData) async {
  ...
  if (distance >= 0.8) {
    //if ED from login attempt and prediction is small it is most likely the same person
    try {
    ...
    // if ED from login attempt and prediction is greater than 0.05 than it is most likely not the same person
...
Future<Map<String, dynamic>?> fetchUserByFace(List<double> face) async {
  final usersCollection = FirebaseFirestore.instance.collection('users');
  final querySnapshot =
      await usersCollection.where('faceData', isEqualTo: face).limit(1).get();
